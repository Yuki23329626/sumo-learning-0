...
cd sumo-learning
git pull
cp ./ns3lte/lteudpMultiUdp.cc ../scratch/
cp ./test10/test10.tcl ../scratch/
cd ..

isSdnEnabled=true
prefixName="lteudpMultiUdpSdn"
...
do
    ...
    echo "endUe = ${endUe}"
    (./waf --run "scratch/lteudpMultiUdp --numberOfUes=100 --startUe=${startUe} --endUe=${endUe} --isSdnEnabled=${isSdnEnabled} --isDownlink=true --isUplink=true --animFile=${animFile} --interAppInterval=0.01 --interPacketInterval=50" > $outLogPath 2>&1) &
    sleep 1s
done
